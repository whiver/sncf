...
const transformLeg = (l) => {
	const leg = {
		origin: {
			type: 'station',
			id: l.originDetail.stationCode,
			name: l.originDetail.stationLabel,
			regions: l.originDetail.cityCode ? [l.originDetail.cityCode] : null,
			url: l.originDetail.stationUrl,
			inFrance: !l.originDetail.outsideFrance
			// todo: other keys
		},
		destination: {
			type: 'station',
			id: l.destinationDetail.stationCode,
			name: l.destinationDetail.stationLabel,
			regions: l.destinationDetail.cityCode ? [l.destinationDetail.cityCode] : null,
			url: l.destinationDetail.stationUrl,
			inFrance: !l.destinationDetail.outsideFrance
			// todo: other keys
		},
...
const transformJourney = (j) => ({
	type: 'journey',
	id: j.id,
	legs: j.segments.filter(l => l.origin && l.destination).map(transformLeg), // fixes weird empty-leg-bug
...
const journeys = async (origin, destination, date, opt) => {
	...
	const list = results.results.map(transformJourney);
